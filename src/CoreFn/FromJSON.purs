...
import Control.Alt ((<|>))
import Control.Monad.Except (except, mapExceptT, runExceptT)
...
import Foreign (F, FT, Foreign, ForeignError(..), fail, readArray, readBoolean, readChar, readInt, readNull, readNumber, readString, typeOf)
import Foreign.Index (index, readIndex, readProp)
import Foreign.JSON (parseJSON)
import Foreign.Keys (keys)

type F' = FT Trampoline
...
moduleFromJSON x = do
  y <- parseJSON x
  except $ runTrampoline $ runExceptT $ moduleFromJSON' y
  where
